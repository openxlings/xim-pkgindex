package = {
    ...
    status = "stable",
    categories = {"library", "embedded", "riscv", "libc"},
    keywords = {"picolibc", "libc", "riscv", "riscv32", "riscv64", "baremetal",
                "freestanding", "embedded", "semihosting", "compiler-rt"},

    -- No programs: this is a sysroot, not a tool. `config()` registers only the
    -- umbrella node so `xlings use picolibc-riscv@<ver>` works.
    xvm_enable = true,

    -- ⭐ The payload is TARGET code (riscv32/riscv64 static archives, object
    -- files and linker scripts), so it is host-independent: ONE archive serves
    ...
    --
    -- Built from source by `.agents/tools/build-baremetal-sysroot.sh` with the
    -- index's own llvm payload, not carved from someone else's distribution:
    -- no upstream publishes a picolibc binary for rv64gc (measured -- upstream
    -- ships prebuilts for arm-none-eabi and aarch64-none-elf only). The script
    -- is the reproduction: same inputs, same bytes (fixed tar owner/mtime and
    -- sorted member order).
    --
    ...
    xpm = {
        source = {
            GLOBAL = "https://github.com/xlings-res/picolibc-riscv/releases/download/${version}/picolibc-riscv-${version}.tar.gz",
            CN = "https://gitcode.com/xlings-res/picolibc-riscv/releases/download/${version}/picolibc-riscv-${version}.tar.gz",
        },
        linux = {
            ["latest"] = { ref = "1.8.12" },
            ["1.8.12"] = { sha256 = "9f577f657048e68c64f146bdd36ce4217ce93ac6c3aec8c13abee26737b0b5c2" },
        },
        macosx = {
            ["latest"] = { ref = "1.8.12" },
            ["1.8.12"] = { sha256 = "9f577f657048e68c64f146bdd36ce4217ce93ac6c3aec8c13abee26737b0b5c2" },
...
local profiles = {
    -- qemu `virt`; the mcpp bare-metal default
    {march = "rv64gc",   mabi = "lp64d", arch = "riscv64"},
    -- what most real RISC-V MCUs are
    {march = "rv32imac", mabi = "ilp32", arch = "riscv32"},
}
...
function install()
    ...
    if not os.isdir(extracted) then
        raise("picolibc-riscv payload not found at " .. extracted)
    end
    os.mv(extracted, dir)

    -- Assert the payload per profile rather than trusting the archive. The
    -- failure this catches is specific and otherwise invisible until a user's
    -- first bare-metal printf: picolibc's ryu float formatting needs 128-bit
    -- shifts, rv64 has no instruction for them, and without
    -- libclang_rt.builtins the link dies on __ashlti3/__lshrti3. Note the
    -- usual "does 64-bit division work" check does NOT reach this -- rv64gc
    -- has a hardware `divu`.
    for _, p in ipairs(profiles) do
        ...
        for _, f in ipairs(required) do
            if not os.isfile(path.join(libdir, f)) then
                raise("picolibc-riscv payload is missing lib/"
                      .. p.march .. "/" .. p.mabi .. "/" .. f)
            end
        end
        if not os.isfile(path.join(incdir, "stdio.h")) then
            raise("picolibc-riscv payload is missing include/"
...
function config()
    ...
    -- usr/include). They are `riscv*-none-elf` TARGET headers: putting them on
    -- the host include path would shadow the host libc for every ordinary
