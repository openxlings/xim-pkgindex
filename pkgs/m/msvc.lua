...
package = {
    ...
    xpm = {
        windows = {
            ...
            -- on the host.
            deps = { "xim:windows-sdk@10.0.26100" },
...
-- Download one payload and prove it is the file we asked for. The check is the
-- point: these bytes become a compiler, and "curl exited 0" says nothing about
-- what arrived.
...
function install()
    ...
    for _, e in ipairs(PAYLOADS) do
        ...
        os.mkdir(stage)
        os.execv("tar", {"-xf", path.join(work, e.name), "-C", stage})
        ...
        if os.isdir(contents) then
            for _, sub in ipairs(os.dirs(path.join(contents, "*"))) do
                local dest = path.join(idir, path.filename(sub))
                if os.isdir(dest) then
                    -- Payloads share the VC/ tree; merge rather than replace.
