...
-- The windows launcher, same contract: `set` is process-local, so NVM_HOME
-- and NVM_SYMLINK are pinned for nvm.exe without touching the user's
-- environment and without going through the merging `envs` path. Anyone who
-- previously ran nvm-setup.exe has NVM_HOME pointing at that install; without
-- this, nvm.exe would read their settings.txt instead of ours.
local function windows_launcher(nvm_dir)
...
function install()
    ...
    if os.host() == "windows" then
        ...
        -- Portable nvm-windows is configured by settings.txt next to nvm.exe.
        -- `path` is where it drops the "active version" symlink; keeping that
        -- inside install_dir is what makes this drop isolated instead of
        -- landing in C:\Program Files\nodejs like the installer does.
        io.writefile(path.join(install_dir, "settings.txt"), string.format(
            "root: %s\r\npath: %s\\nodejs\r\narch: 64\r\nproxy: none\r\n",
            win_dir, win_dir
        ))

        io.writefile(path.join(install_dir, "nvm.cmd"), windows_launcher(win_dir))
    else
