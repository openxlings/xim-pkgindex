-- cuda-crt — the NVIDIA CUDA redistributable component `cuda_crt`.
--
...
package = {
    ...
    name = "cuda-crt",
    description = "CUDA cuda_crt: crt/host_config.h and the compiler runtime headers",
    ...
    keywords = {"cuda", "nvidia", "gpu", "cuda_crt"},
    ...
    xpm = {
        linux = {
            ...
            source = "https://developer.download.nvidia.com/compute/cuda/redist/"
                  .. "cuda_crt/linux-${arch_alias}/"
                  .. "cuda_crt-linux-${arch_alias}-${version}-archive.tar.xz",
            arch_alias = { x86_64 = "x86_64", aarch64 = "sbsa" },
            ["latest"] = { ref = "13.3.33" },
            ["13.3.33"] = {
                sha256 = {
                    aarch64  = "6f6194918c00b980d8fd2111bf0aa004977760855c6e1528e0653bf4c889fbef",
                    x86_64   = "4755d36d24c6ef7697a2d3e1dbb23c4562c9c0d97d48390d4cbd8ab32dec5b5f",
                },
            },
        },
    },
...
-- The archive unpacks to `cuda_crt-linux-<arch>-<version>-archive/`.
...
function install()
    ...
    if not src then
        error("cuda-crt: the downloaded archive did not unpack to a recognisable "
              .. "component directory")
    end
    local dir = pkginfo.install_dir()
    os.tryrm(dir)
    os.mv(src, dir)
    log.info("cuda-crt installed to %s", dir)
    return true
end
