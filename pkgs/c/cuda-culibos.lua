-- cuda-culibos — the NVIDIA CUDA redistributable component `cuda_culibos`.
--
...
package = {
    ...
    name = "cuda-culibos",
    description = "CUDA cuda_culibos: the OS abstraction the static CUDA runtime links against",
    ...
    keywords = {"cuda", "nvidia", "gpu", "cuda_culibos"},
    ...
    xpm = {
        linux = {
            ...
            source = "https://developer.download.nvidia.com/compute/cuda/redist/"
                  .. "cuda_culibos/linux-${arch_alias}/"
                  .. "cuda_culibos-linux-${arch_alias}-${version}-archive.tar.xz",
            arch_alias = { x86_64 = "x86_64", aarch64 = "sbsa" },
            ["latest"] = { ref = "13.3.33" },
            ["13.3.33"] = {
                sha256 = {
                    aarch64  = "af8e930c3527b8eeddea755b40d77e505691ca389f3135d04f46718aec3f45cb",
                    x86_64   = "db36b1381bb3105f2f13591393131d93f501958ebd44e9d0d52234cbbaf8cbd0",
                },
            },
        },
    },
...
-- The archive unpacks to `cuda_culibos-linux-<arch>-<version>-archive/`.
...
function install()
    ...
    if not src then
        error("cuda-culibos: the downloaded archive did not unpack to a recognisable "
              .. "component directory")
    end
    local dir = pkginfo.install_dir()
    os.tryrm(dir)
    os.mv(src, dir)
    log.info("cuda-culibos installed to %s", dir)
    return true
end
