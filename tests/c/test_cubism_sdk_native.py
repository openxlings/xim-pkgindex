"""Tests for the cubism-sdk-native package."""
import re
...
PKG = "cubism-sdk-native"
PKG_FILE = "pkgs/c/cubism-sdk-native.lua"
...
class TestStatic:
    ...
    def test_no_cn_mirror_entry(self, meta):
        ...
        would republish Live2D's proprietary Core binaries."""
        code = _code(meta.raw_content)
    ...
    def test_upstream_url_and_sha256_are_pinned(self, meta):
        code = _code(meta.raw_content)
        assert "https://cubism.live2d.com/sdk-native/bin/CubismSdkForNative-5-r.5.zip" in code
        assert "7ff3a4bbc19c0a8728965aa522ab77eb11b252916453e68a8a78d3b71188bb12" in code
    ...
    @pytest.mark.static
    def test_install_verifies_all_three_licensed_trees(self, meta):
        ...
        assert "Core/include/Live2DCubismCore.h" in install_body
        assert "Framework/src/CubismFramework.hpp" in install_body
        assert "Core/LICENSE.md" in install_body
        assert install_body.count("os.isfile") >= 3, \
    ...
    def test_bare_registration_no_bindir(self, meta):
        ...
        assert re.search(r'xvm\.add\s*\(\s*"cubism-sdk-native"\s*\)', config_body), \
