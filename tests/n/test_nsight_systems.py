...
PKG = "nsight-systems"
PKG_FILE = "pkgs/n/nsight-systems.lua"
...
class TestStatic:
    ...
    def test_version_is_upstream(self, meta):
        ...
        Nsight 按年编号(2026.1.3.243),与 CUDA 组件的 13.3.33 形状不同。
        判据因此钉具体取值而不是形状:用形状写的那一版把这两个包判成了
        「不像上游版本」,而它们恰恰是上游版本。
        """
        vers = re.findall(r'\["(\d[^"]*)"\]\s*=', meta.raw_content)
        assert "2026.1.3.243" in vers, (
            "nsight-systems must carry upstream version 2026.1.3.243; found %s" % vers
        )
