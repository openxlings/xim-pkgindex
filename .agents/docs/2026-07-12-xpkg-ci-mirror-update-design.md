...
| `ci.update` | `false` / duration string | `false` | 按周期扫描固定 URL template，发现新版本后创建更新 PR |

允许的周期字符串为 `1d`、`3d`、`7d`、`14d`、`30d`。最短周期是 `1d`，默认推荐
`3d`。不引入 `target`、`strategy`、`submit` 等字段：当前唯一官方目标是
`xlings-res`，更新方式固定为 PR。
...
两者独立：`mirror=true` 不会发现新版本；`update="3d"` 不会绕过 PR 直接修改
...
职责：解析 `package.ci`、验证类型、规范化周期、识别 source/template、检查平台和
...
3. `update="3d"` 最多每三天扫描一次，并且只创建 PR。
