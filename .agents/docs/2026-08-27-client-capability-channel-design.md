...
>
> **放在索引仓库,因为提出这个问题的人在这里。** 通道真要建的话代码大半在
> libxpkg 和 xlings,但会问"我这个 recipe 怎么知道客户端会不会回收"的是写
> recipe 的人,而他们是在读 `libs/sysroot.lua` 时撞上它的;最后做出的动作
> (#694 删掉手写清理)也整个在这个仓库里。
>
> 唯一一条属于 xlings 的结论是 §2.4(4 个 `ExecutionContext` 建造点需要收敛
> 成一个写入者)—— 它与通道做不做无关,已单独开 issue,不靠这份文档存活。
...
真要建的时候,这些结论不用重新推一遍。§2.4 那条(4 个 `ExecutionContext` 建造点必须收敛)与通道无关也成立,
已单独开 openxlings/xlings#566:现在 `subos_sysrootdir` 已经在那 4 处
分成了两种填法。真要建通道时,那个工厂是**前置条件**而不是可选项 ——
能力必须在所有 ExecutionContext 上都出现,漏一处就是静默走回退。
