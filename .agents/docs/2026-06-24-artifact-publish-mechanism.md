...
1. **P0 ✅ 已实现**(PR #301/#302/#303,已 squash 合入):本仓 `.github/workflows/publish-artifact.yml`
   (push `pkgs/**` / 手动 / nightly)+ vendored `tools/{build_xim_index_artifact,publish_xim_index,push_index_pointers}.sh`
   + `tools/gtc` + 内容哈希命名(`xim-index-<short-sha>.tar.gz`)。
   - 实测 dispatch:产出 `xim-index-2f4ad3e.tar.gz` → 发到 `xlings-res/xim-index`(github `latest`+`v<sha>`、gitcode 同)
     → 合并指针 `xim-index-pointers.json` 两端,keys `[awesome,d2x,scode,xim]` 保留。
   - **修复(#303)**:`push_index_pointers.sh` 改为**合并自己的 key**(不覆盖兄弟索引)——
     per-repo 单独发布时只有自己的 `*-latest.json`,旧的整体覆盖会丢掉 awesome/scode/d2x。
2. **P1**:客户端刷新走指针 sha 比对(见 xlings 仓文档)。
3. **P2**:`mcpp-community/mcpp-index` 同款(发到 `xlings-res/mcpp-index`,见 mcpp-index 仓文档)。

> 待办:把同样的"合并指针"修复同步回 xlings 仓 `tools/push_index_pointers.sh`(release 路径目前
> 一次构建全部 index 故覆盖无害,但合并更安全、避免与 per-repo CI 竞争)。
